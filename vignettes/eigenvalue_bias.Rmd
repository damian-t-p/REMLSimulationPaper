...
save_data = TRUE & perform_simulations
```
...
v_row <- function(m) {
  ...
  r <- floor(m/2)
  downslope <- (1 - (1:r)/(r+1))/r
  out <- rep(1, m)
  
  out[2:(r+1)] <- downslope
  out[(m-r+1):m] <- rev(downslope)
...
clinear <- function(p, d) {
  m <- p - d
  v <- v_row(m)

  circ_block <- t(sapply(0:(m-1), \(i) rot(v, i)))

  mat_out <- matrix(0, nrow = p, ncol = p)

  if (m > 0) {
    mat_out[1:m, 1:m] <- circ_block
  }
...
a_types <- list(
  ...
  chisquared = function(p, d) diag(c(rchisq(p - d, df=5), rep(0, d))),
...
a_sub <- c("identity", "chisquared", "clinear")
